...
collapsible channel where the flow is driven by a pulsatile
pressure difference. We show that the
membrane oscillation exhibits strong resonances, reminiscent of a
forced damped harmonic oscillator. Guided by this analogy, we derive
...
through a two-dimensional channel of length $l$ and height $a$.
The lower channel wall is rigid, whereas a pre-stressed, elastic
membrane of length $l_\text{m}$ is clamped between two rigid segments
at the upper wall and is pressurized by an external
pressure $P_\text{ext}$.

The fluid motion is governed by the incompressible Navier--Stokes
equations, which in dimensionless form read
\begin{equation}
Re \left(\dfrac{\partial u_j}{\partial t} + u_i\frac{\partial u_j
}{\partial x_i} \right) = -\frac{\partial p}{\partial x_j} +
...
$a$, the velocities on  ${\cal U} = P_0 a^2/(12 \mu
...
\begin{equation}
  P(t)= \dfrac{12\,l}{a}\left(1 + \sin\,(\omega \, t)\right),
\end{equation}
and the pressure at the downstream end of the channel is set to zero.
...
period of the imposed pressure variation. 
The boundary conditions for the velocity are no-slip on the walls,
...
membrane (of thickness $h$ and Young's modulus $E$, subject to a
...
f_j=-P_\text{ext} n_j + Q \left[ p n_j - \left(\frac{\partial u_i}{\partial x_j} + \frac{\partial u_j}{\partial x_i}\right) n_i\right].
\end{equation}
Here ${n_j}$ is the outer normal to the membrane,
$P_\text{ext}=p_\text{ext}/E$ is the dimensionless external
pressure, and $Q=\mu {\cal U}/(aE)$ is a non-dimensional parameter
that represents the ratio of the typical viscous stresses in the fluid
to the elastic modulus of the membrane.
...
given by $R_i(\xi,t) = r_i(\xi) + d_i(\xi,t)$. Here $r_i(\xi) = [\xi,
  1]^T$ defines the undeformed configuration and $d_i(\xi,t)$ is the
displacement vector. The membrane deformation is governed by the
principle of virtual displacements
\begin{equation}
\label{eq:virtualdisplacement}
\int_{0}^{\frac{l_\text{m}}{a}} \left((\sigma_0 + \gamma)\delta \gamma
+ \frac{1}{12}\epsilon^2\kappa \,\delta\kappa -
\frac{1}{\epsilon}{f_j} \delta {R}_j \Lambda\right) d\xi = 0,
\end{equation}
where $\epsilon=h/a$ is the dimensionless thickness of the membrane,
$\gamma=\partial d_1/\partial {\xi} + \frac{1}{2}[(\partial d_1/\partial
  {\xi})^2 + (\partial d_2/\partial {\xi})^2]$ the strain tensor, and
...
We investigate the dynamics of the system by monitoring the vertical
displacement of the membrane at its midpoint, $y(t)$, as a proxy for the
deformation. We start the simulations from an initial condition in
...
the decay of initial transients the system ultimately settles into a
...
illustrated in Fig.~\ref{fig:extpressure}(a) which shows time traces of $y(t)$
...
that the amplitude of the time-periodic oscillation, $\widehat{y}$, is
independent of the external pressure (and from now on we set $P_\text{ext}=0.1$).
...
Figs. \ref{fig:extpressure}(c-f) illustrate... {\bf *** Note: these plots were
...
non-dimensonal forcing frequency $\omega$ for five different
Reynolds numbers while keeping $H = Re/Q = ****$ constant.
...
passes through $\omega_0.$ {\bf *** please do these computations and
  update figures ***}
